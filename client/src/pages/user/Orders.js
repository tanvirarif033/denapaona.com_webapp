  ...
  const getOrders = async () => {
    try {
      const { data } = await axios.get(
        "https://denapaona-com-webapp-server.vercel.app/api/v1/auth/orders"
      );

      // Check if data.orders is an array before setting the state
      if (Array.isArray(data.orders)) {
        setOrders(data.orders);
      } else {
        console.error("Unexpected response format, orders not an array", data);
        setOrders([]); // Set empty array if orders are not returned correctly
      }
    } catch (error) {
      console.log(error);
    }
